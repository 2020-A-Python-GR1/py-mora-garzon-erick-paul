...
class AraniaCrawlFybeca(CrawlSpider):
    ...
    start_urls = [
        ...
        'https://www.fybeca.com/FybecaWeb/pages/search-results.jsf?cat=457&s=0&pp=25&q='
    ]
    ...
    def parse_page(self, response):
        ...
        for (nombre, precio,precio_afiliado) in zip(nombres, precios_final,precios_miembro_final): 
            with open('fybeca_items.txt', 'a+', encoding='utf-8') as archivo:
                archivo.write(nombre + '\t'+'Afiliado: $'+precio_afiliado+ '\t\t'+'No afiliado: $'+ precio+ '\t' +'Ahorro: $'+str(round((float(precio) - float(precio_afiliado)),2))+'\n')